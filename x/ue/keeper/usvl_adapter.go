...
// UtvKeeperAdapter adapts the utv.Keeper to match the ue.UtvKeeper interface
type UtvKeeperAdapter struct {
	keeper utvkeeper.Keeper
}

// NewUtvKeeperAdapter creates a new adapter for the UTV keeper
func NewUtvKeeperAdapter(keeper utvkeeper.Keeper) UtvKeeperAdapter {
	return UtvKeeperAdapter{
		keeper: keeper,
	}
}

// VerifyExternalTransaction verifies transactions by delegating to the actual UTV keeper
// and converting the response type to match the ue module's expected type
func (a UtvKeeperAdapter) VerifyExternalTransaction(ctx context.Context, txHash string, caipAddress string) (*types.UtvVerificationResult, error) {
	// Call the actual UTV keeper implementation
	...
	return &types.UtvVerificationResult{
...
// by delegating to the actual UTV keeper and converting the response type
func (a UtvKeeperAdapter) VerifyExternalTransactionToLocker(ctx context.Context, txHash string, caipAddress string) (*types.UtvVerificationResult, error) {
	// Call the actual UTV keeper implementation
...
func (a UtvKeeperAdapter) GetFundsAddedEventTopic(ctx context.Context, chainIdentifier string) (string, error) {
