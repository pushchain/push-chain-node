...
func handleBalance(cfg config.Config, args []string) {
    ...
    if addr == "" {
        ...
        if key == "" {
        if flagOutput == "json" { getPrinter().JSON(map[string]any{"ok": false, "error": "address not provided; set KEY_NAME or pass --address"}) } else { fmt.Println("usage: push-validator-manager balance <address> (or set KEY_NAME)") }; os.Exit(1)
    }
