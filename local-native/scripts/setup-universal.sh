...
# Fix Solana gateway program: default_config has wrong protocol_alt (6AdUqeK...) but the
# actual deployed program in Surfnet is CFVSinc... (executable, has the TSS PDA initialized)
jq '.chain_configs["solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1"].protocol_alt = "CFVSincHYbETh2k7w6u1ENEkjbSLtveRCEBupKidw2VS"' \
   "$HOME_DIR/config/pushuv_config.json" > "$HOME_DIR/config/pushuv_config.json.tmp" && \
   mv "$HOME_DIR/config/pushuv_config.json.tmp" "$HOME_DIR/config/pushuv_config.json"
