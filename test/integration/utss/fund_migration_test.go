...
func seedFundMigrationChainValues(
	...
	_, err = chainApp.EVMKeeper.CallEVM(ctx, chainApp.EVMKeeper.NewStateDB(ctx), setupABI, admin, handlerAddr, true, false, nil, "grantRole", roleArg, admin)
	require.NoError(t, err, "grant MANAGER_ROLE")

	_, err = chainApp.EVMKeeper.CallEVM(ctx, chainApp.EVMKeeper.NewStateDB(ctx), setupABI, admin, handlerAddr, true, false, nil, "setTssFundMigrationGasLimitByChain", chain, gasLimit)
	require.NoError(t, err, "seed tss fund migration gas limit")

	_, err = chainApp.EVMKeeper.CallEVM(ctx, chainApp.EVMKeeper.NewStateDB(ctx), setupABI, admin, handlerAddr, true, false, nil, "setL1GasFeeByChain", chain, l1GasFee)
