...
func (mpd MinGasPriceDecorator) AnteHandle(
	...
	// ctx.Logger().Info("Starting MinGasPriceDecorator",
	// 	"height", ctx.BlockHeight(),
	// 	"tx_type", fmt.Sprintf("%T", tx))

	if mpd.feesKeeper == nil {
		ctx.Logger().Error("feesKeeper is nil")
		return ctx, errorsmod.Wrap(errortypes.ErrLogic, "feesKeeper not initialized")
	}
	if mpd.evmKeeper == nil {
		ctx.Logger().Error("evmKeeper is nil")
		return ctx, errorsmod.Wrap(errortypes.ErrLogic, "evmKeeper not initialized")
	}

	feeTx, ok := tx.(sdk.FeeTx)
	if !ok {
		return ctx, errorsmod.Wrapf(errortypes.ErrInvalidType, "tx must implement FeeTx, got %T", tx)
	}

	if ctx.BlockHeight() == 0 {
		// ctx.Logger().Info("Skipping min gas price check at genesis height")
	...
	// Skip for MsgCreateValidator at genesis
	if len(tx.GetMsgs()) == 1 {
		if _, ok := tx.GetMsgs()[0].(*stakingtypes.MsgCreateValidator); ok && ctx.BlockHeight() == 0 {
			// ctx.Logger().Info("Skipping min gas price check for MsgCreateValidator at genesis")
			return next(ctx, tx, simulate)
		}
	}

	params := mpd.feesKeeper.GetParams(ctx)
	minGasPrice := params.MinGasPrice
	// ctx.Logger().Info("Checking min gas price",
	// 	"required_min_gas_price", minGasPrice,
	// 	"tx_gas_limit", feeTx.GetGas(),
	// 	"tx_fee", feeTx.GetFee())
