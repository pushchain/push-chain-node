...
type pt struct{ h int64; t time.Time }

// Run performs two-phase monitoring: snapshot spinner from logs, then WS header progress.
func Run(ctx context.Context, opts Options) error {
    if opts.Out == nil { opts.Out = os.Stdout }
    if opts.Window <= 0 { opts.Window = 30 }
    ...
    close(stopLog)
    if phase1Err != nil {
        if tty { fmt.Fprint(opts.Out, "\r\033[K") }
        return phase1Err
    }
    if tty { fmt.Fprint(opts.Out, "\r\033[K") }
    if sawAccepted && tty {
    ...
    if err != nil { return fmt.Errorf("ws subscribe: %w", err) }

    // Remote poller
    remote := strings.TrimRight(opts.RemoteRPC, "/")
    if remote == "" { remote = local }
    remoteHCh := make(chan int64, 1)
    go pollRemote(ctx, remote, 4*time.Second, remoteHCh)
    ...
    for {
        ...
        case <-ctx.Done():
            return ctx.Err()
        case rh := <-remoteHCh:
            if rh > 0 { lastRemote = rh }
        case h, ok := <-headers:
            if !ok { return nil }
            buf = append(buf, pt{h.Height, time.Now()})
            if len(buf) > opts.Window { buf = buf[1:] }
            // Render progress
            var cur = h.Height
            ...
            if baseH == 0 && lastRemote > 0 && len(buf) > 0 {
                ...
                if baseRemote <= baseH { baseRemote = baseH + 1000 }
            }
            var percent float64
            if lastRemote > 0 {
                if baseH > 0 && lastRemote > baseH {
                    denom := float64(lastRemote - baseH)
                    if denom > 0 { percent = float64(cur-baseH) / denom * 100 }
                } else {
                    percent = float64(cur) / float64(lastRemote) * 100
                }
            }
            // Avoid rounding up to 100.0 before actually matching remote
            ...
            if !barPrinted { firstBarTime = time.Now() }
            barPrinted = true
        case <-tick.C:
            ...
            if err == nil {
                ...
                if !barPrinted {
                    ...
                    if remoteH < cur { remoteH = cur }
                    // If already synced but local height not yet reported, align to remote
                    if !st.CatchingUp && cur < remoteH {
                        cur = remoteH
                    }
                    // Avoid printing an empty bar when both are zero; wait for next tick
                    if cur == 0 && remoteH == 0 { break }
                    var percent float64
                    if remoteH > 0 { percent = float64(cur) / float64(remoteH) * 100 }
                    percent = floor1(percent)
                    if cur < remoteH && percent >= 100 { percent = 99.9 }
                    line := renderProgress(percent, cur, remoteH)
                    if tty { fmt.Fprintf(opts.Out, "\r\033[K%s", line) } else { fmt.Println(line) }
                    firstBarTime = time.Now()
                    barPrinted = true
                }
                if !st.CatchingUp {
                    // Keep the bar visible for at least 5 seconds so users can see it
                    if barPrinted && time.Since(firstBarTime) < 5*time.Second {
                        // Update bar using current local status while we hold
                    cur := st.Height
                    if cur > 0 {
                        // Refresh remote height each tick during the hold to keep the bar moving
                        remoteH := probeRemoteOnce(opts.RemoteRPC, cur)
                        if remoteH < cur { remoteH = cur }
                        percent := 0.0
                        if remoteH > 0 { percent = float64(cur) / float64(remoteH) * 100 }
                    ...
                    if tty { fmt.Fprint(opts.Out, "\n✅ Node is fully synced!\n") } else { fmt.Fprintln(opts.Out, "Node is fully synced.") }
                    return nil
                }
            }
        }
...
func tailStatesync(ctx context.Context, path string, out chan<- string, stop <-chan struct{}) {
    ...
    defer f.Close()
    // Seek to end
    if _, err := f.Seek(0, io.SeekEnd); err != nil { return }
    r := bufio.NewReader(f)
    for {
        select {
        case <-ctx.Done():
...
func hostPortFromURL(s string) string {
    u, err := url.Parse(s)
    if err == nil && u.Host != "" { return u.Host }
...
func isSyncedQuick(local string) bool {
    ...
    var payload struct{ Result struct{ SyncInfo struct{ CatchingUp bool `json:"catching_up"` } `json:"sync_info"` } `json:"result"` }
    ...
    for {
        ...
        case <-time.After(every):
            ...
            if payload.Result.SyncInfo.Height != "" {
                ...
                if hv > 0 { select { case out <- hv: default: } }
            }
        }
    }
}
...
func probeRemoteOnce(base string, fallback int64) int64 {
    ...
    defer resp.Body.Close()
    var payload struct{ Result struct{ SyncInfo struct{ Height string `json:"latest_block_height"` } `json:"sync_info"` } `json:"result"` }
    if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil { return fallback }
...
func renderProgress(percent float64, cur, remote int64) string {
    ...
    return fmt.Sprintf("📊 Syncing [%s] %.1f%% | %d/%d blocks", bar, percent, cur, remote)
}
...
func hideCursor(w io.Writer, tty bool) { if tty { fmt.Fprint(w, "\x1b[?25l") } }
func showCursor(w io.Writer, tty bool) { if tty { fmt.Fprint(w, "\x1b[?25h") } }
...
func movingRatePt(in []pt) float64 {
    tmp := make([]struct{ h int64; t time.Time }, len(in))
    for i := range in { tmp[i] = struct{ h int64; t time.Time }{h: in[i].h, t: in[i].t} }
