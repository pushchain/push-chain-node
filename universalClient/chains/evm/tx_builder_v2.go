...
// DefaultGasLimitV2 is used when gas limit is not provided in the outbound event data
const DefaultGasLimitV2 = 500000
...
// RevertInstructionsV2 represents the struct for revert instruction in V2 contracts
// Matches: struct RevertInstructions { address revertRecipient; bytes revertMsg; }
type RevertInstructionsV2 struct {
	RevertRecipient ethcommon.Address
	RevertMsg       []byte
}

// TxBuilderV2 implements OutboundTxBuilder for EVM chains using Vault + Gateway V2 contracts.
...
// NewTxBuilderV2 creates a new EVM transaction builder for Vault + Gateway V2.
...
func NewTxBuilderV2(
	...
	tb := &TxBuilderV2{
		...
		logger:         logger.With().Str("component", "evm_tx_builder_v2").Str("chain", chainID).Logger(),
	}
...
func (tb *TxBuilderV2) getVaultAddress() (ethcommon.Address, error) {
	tb.vaultMu.RLock()
...
func (tb *TxBuilderV2) tryRefreshVaultAddress() {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (tb *TxBuilderV2) fetchVaultAddress(ctx context.Context) (ethcommon.Address, error) {
...
func (tb *TxBuilderV2) GetOutboundSigningRequest(
	...
	gasLimit := parseGasLimitV2(data.GasLimit)
	...
	txType, err := parseTxTypeV2(data.TxType)
	if err != nil {
		return nil, fmt.Errorf("invalid tx type: %w", err)
	}

	funcName := tb.determineFunctionNameV2(txType, assetAddr)

	txData, err := tb.encodeFunctionCallV2(funcName, data, amount, assetAddr, txType)
	if err != nil {
...
func (tb *TxBuilderV2) GetNextNonce(ctx context.Context, signerAddress string, useFinalized bool) (uint64, error) {
	if signerAddress == "" {
...
func (tb *TxBuilderV2) BroadcastOutboundSigningRequest(
	...
	gasLimitForTx := parseGasLimitV2(data.GasLimit)
...
func (tb *TxBuilderV2) VerifyBroadcastedTx(ctx context.Context, txHash string) (found bool, blockHeight uint64, confirmations uint64, status uint8, err error) {
...
func (tb *TxBuilderV2) resolveTxParams(funcName string, assetAddr ethcommon.Address, amount *big.Int) (txValue *big.Int, toAddress ethcommon.Address, err error) {
...
// determineFunctionNameV2 determines the function name based on TxType and asset type.
//
// V2 routing:
//   - FUNDS, FUNDS_AND_PAYLOAD, PAYLOAD → Vault.finalizeUniversalTx
//   - INBOUND_REVERT (native)           → Gateway.revertUniversalTx
//   - INBOUND_REVERT (ERC20)            → Vault.revertUniversalTxToken
func (tb *TxBuilderV2) determineFunctionNameV2(txType uetypes.TxType, assetAddr ethcommon.Address) string {
	isNative := assetAddr == (ethcommon.Address{})
...
// encodeFunctionCallV2 encodes the function call based on V2 contract ABIs
func (tb *TxBuilderV2) encodeFunctionCallV2(
	funcName string,
	data *uetypes.OutboundCreatedEvent,
	amount *big.Int,
	...
	txIDBytes, err := hex.DecodeString(removeHexPrefixV2(data.TxID))
	if err != nil {
		return nil, fmt.Errorf("invalid txID: %s", data.TxID)
	}

	universalTxIDBytes, err := hex.DecodeString(removeHexPrefixV2(data.UniversalTxId))
	...
	payloadBytes, err := hex.DecodeString(removeHexPrefixV2(data.Payload))
	if err != nil {
		payloadBytes = []byte{}
	}

	isNative := assetAddr == (ethcommon.Address{})
	funcSignature := tb.getFunctionSignatureV2(funcName, isNative)
	...
	case "revertUniversalTx":
		...
		revertMsgBytes, err := hex.DecodeString(removeHexPrefixV2(data.RevertMsg))
		if err != nil {
			revertMsgBytes = []byte{}
		}
		...
		values = []interface{}{txID32, universalTxID, amount, RevertInstructionsV2{
	...
	case "revertUniversalTxToken":
		...
		revertMsgBytesToken, err := hex.DecodeString(removeHexPrefixV2(data.RevertMsg))
		if err != nil {
		...
		values = []interface{}{txID32, universalTxID, assetAddr, amount, RevertInstructionsV2{
			RevertRecipient: target,
...
// getFunctionSignatureV2 returns the full function signature for ABI encoding
func (tb *TxBuilderV2) getFunctionSignatureV2(funcName string, _ bool) string {
	switch funcName {
	case "finalizeUniversalTx":
...
// removeHexPrefixV2 removes the 0x prefix from a hex string
func removeHexPrefixV2(s string) string {
	if len(s) >= 2 && s[0:2] == "0x" {
		return s[2:]
	}
	return s
...
// parseTxTypeV2 parses the TxType string to uetypes.TxType enum
func parseTxTypeV2(txTypeStr string) (uetypes.TxType, error) {
...
// parseGasLimitV2 parses gas limit string, returning default if empty or zero
func parseGasLimitV2(gasLimitStr string) *big.Int {
	if gasLimitStr == "" || gasLimitStr == "0" {
		return big.NewInt(DefaultGasLimitV2)
	}
