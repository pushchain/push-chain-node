...
func TestParseGatewayEvent(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			eventType: EventTypeVaultUniversalTxFinalized,
			log: &types.Log{
				Address: gatewayAddr,
...
func TestParseOutboundObservationEvent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			event := ParseEvent(tt.log, EventTypeVaultUniversalTxFinalized, chainID, logger)
...
func TestParseGatewayEvent_OutboundObservation(t *testing.T) {
	...
	t.Run("parses outbound observation event correctly", func(t *testing.T) {
		...
		event := ParseEvent(log, EventTypeVaultUniversalTxFinalized, config.Chain, logger)
		require.NotNil(t, event)
