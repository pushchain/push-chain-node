...
func (suite *KeyringTestSuite) TestGetKeyringKeybaseWithExistingKey() {
	...
	_, _, err := CreateNewKey(suite.kb, "test-key", "", "")
	require.NoError(suite.T(), err)
...
func (suite *KeyringTestSuite) TestCreateNewKey() {
	record, _, err := CreateNewKey(suite.kb, "new-test-key", "", "")
...
func (suite *KeyringTestSuite) TestCreateNewKeyWithMnemonic() {
	...
	record, returnedMnemonic, err := CreateNewKey(suite.kb, "mnemonic-key", mnemonic, "")

	require.NoError(suite.T(), err)
	assert.NotNil(suite.T(), record)
	assert.Equal(suite.T(), "mnemonic-key", record.Name)
	assert.Equal(suite.T(), mnemonic, returnedMnemonic) // Should return the provided mnemonic
...
func (suite *KeyringTestSuite) TestCreateNewKeyWithInvalidMnemonic() {
	...
	_, _, err := CreateNewKey(suite.kb, "invalid-key", invalidMnemonic, "")
...
func (suite *KeyringTestSuite) TestValidateKeyExists() {
	// Create a key first
	_, _, err := CreateNewKey(suite.kb, "validation-test", "", "")
	require.NoError(suite.T(), err)
...
func (suite *KeyringTestSuite) TestGetPubkeyBech32FromRecord() {
	// Create a key
	record, _, err := CreateNewKey(suite.kb, "pubkey-test", "", "")
	require.NoError(suite.T(), err)
...
func (suite *KeyringTestSuite) TestKeyringConfigValidation() {
	...
	_, _, err := CreateNewKey(suite.kb, validConfig.HotkeyName, "", "")
	require.NoError(suite.T(), err)
