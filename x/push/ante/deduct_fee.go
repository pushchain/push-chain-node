...
// isMsgMintTokenTx checks if the transaction contains only MsgMintToken
func isMsgMintTokenTx(tx sdk.Tx) bool {
	msgs := tx.GetMsgs()
	if len(msgs) != 1 {
		return false
	}
	return sdk.MsgTypeURL(msgs[0]) == sdk.MsgTypeURL(&types.MsgMintTokens{})
}
...
func NewDeductFeeDecorator(ak types.AccountKeeper, bk types.BankKeeper, fk types.FeegrantKeeper, tfc TxFeeChecker) DeductFeeDecorator {
	if tfc == nil {
...
func checkTxFeeWithValidatorMinGasPrices(ctx sdk.Context, tx sdk.Tx) (sdk.Coins, int64, error) {
	...
	// If it's a MintToken transaction, return zero fees and priority
	if isMsgMintTokenTx(tx) {
