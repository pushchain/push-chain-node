...
func TestMsgUpdateUniversalValidatorStatus_ValidateBasic(t *testing.T) {
	cfg := sdk.GetConfig()
	...
	validNewStatus := types.UVStatus_UV_STATUS_ACTIVE
	invalidNewStatus := types.UVStatus_UV_STATUS_INACTIVE

	tests := []struct {
		name    string
		msg     types.MsgUpdateUniversalValidatorStatus
		wantErr bool
		errMsg  string
	}{
		{
