...
func (k Keeper) recordReadRequest(
	...
	ur := types.UniversalRead{
		...
		Request: &types.ReadRequest{
			...
			FeesDeposited:     bigOrZero(event.TotalPaid),
			MaxFee:            bigOrZero(event.MaxFee),
			ProtocolFee:       bigOrZero(event.ProtocolFee),
			CallbackBudget:    bigOrZero(event.CallbackBudget),
			CallbackGasLimit:  event.CallbackGasLimit,
			RevertRecipient:   event.RevertRecipient,
