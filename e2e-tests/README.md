...
1. Starts local fork nodes:
   ...
   - `surfpool` for Solana
2. Copies `config/testnet-donut` into `e2e-tests/config/testnet-donut`, then rewrites `public_rpc_url` there to local fork URLs
...
### Quick outbound smoke test

For the fastest outbound sanity check after a fresh bootstrap, chain `all` with `quick-testing-outbound`:
...
- update `e2e-tests/config/testnet-donut/*/tokens/*.json`
...
| `e2e-tests/config/testnet-donut/*/tokens/*.json` | Runtime token config working copy (updated contract addresses) |
