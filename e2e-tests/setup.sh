...
// LOCAL SVM seed patch: the deployed Surfnet gateway program uses seed "tsspda_v2"
// for its TSS PDA, but the Go binary was written expecting "final_tss_pda" (the newer
// program version). Rewrite all three occurrences so the validator passes the correct
// account to FinalizeUniversalTx and also reads from the correct address.
txBuilderSrc = txBuilderSrc.replaceAll('"final_tss_pda"', '"tsspda_v2"');
...
if (!route.includes('LOCAL_SVM_ROUTE3_SPL_PRC20') && !route.includes('resolveR3SvmDrain')) {
...
if (!route.includes('LOCAL_SVM_ROUTE3_BORSH_PAYLOAD') && !route.includes('buildR3SvmExtraPayload')) {
...
if (!route.includes('LOCAL_SVM_ROUTE3_PUSH_RECIPIENT') && !route.includes('buildR3SvmExtraPayload')) {
  const start = route.indexOf('export async function executeCeaToPushSvm');
  // Match either the old 6-arg call (no maxPCForGas) or the new 7-arg call (with maxPCForGas)
  let marker = `    params.gasLimit ?? BigInt(0),
    svmPayload,
    ueaAddress,
    params.maxPCForGas ?? BigInt(0)
  );`;
  let replacement = `    params.gasLimit ?? BigInt(0),
    svmPayload,
    pushPayloadRecipient, // LOCAL_SVM_ROUTE3_PUSH_RECIPIENT
    params.maxPCForGas ?? BigInt(0)
  );`;
  if (!route.includes(marker, start)) {
    // Fall back to old 6-arg form
    marker = `    params.gasLimit ?? BigInt(0),
    svmPayload,
    ueaAddress
  );`;
    replacement = `    params.gasLimit ?? BigInt(0),
    svmPayload,
    pushPayloadRecipient // LOCAL_SVM_ROUTE3_PUSH_RECIPIENT
  );`;
  }
...
step_run_sdk_quick_testing_outbound_evm() {
  local outbound_dir="$PUSH_CHAIN_SDK_DIR/packages/core/__e2e__/evm/outbound"
  local quick_files=(
    "cea-to-eoa.spec.ts"
    "cea-to-uea.spec.ts"
...
step_run_sdk_quick_testing_outbound_svm() {
  ...
  for file in "${quick_files[@]}"; do
    full_path="$outbound_dir/$file"
    if [[ ! -f "$full_path" ]]; then
      log_err "SDK outbound SVM test file not found: $full_path"
      exit 1
    fi
    perl -0pi -e 's/\bPUSH_NETWORK\.TESTNET_DONUT\b/PUSH_NETWORK.LOCALNET/g; s/\bPUSH_NETWORK\.TESTNET\b/PUSH_NETWORK.LOCALNET/g; s/\bCHAIN\.PUSH_TESTNET_DONUT\b/CHAIN.PUSH_LOCALNET/g' "$full_path"
    log_ok "Prepared LOCALNET network replacement in svm/outbound/$file"
  done
  ...
  if [[ -f "$svm_client_file" ]]; then
    perl -0pi -e 's/\bPUSH_NETWORK\.TESTNET_DONUT\b/PUSH_NETWORK.LOCALNET/g' "$svm_client_file"
    log_ok "Patched svm-client.ts default network to PUSH_NETWORK.LOCALNET"
  fi
  if [[ -f "$utils_file" ]]; then
    perl -0pi -e 's/(const network = options\?\.network \?\?)\s*PUSH_NETWORK\.TESTNET_DONUT/$1 PUSH_NETWORK.LOCALNET/' "$utils_file"
    log_ok "Patched utils.ts getPRC20Address default network to PUSH_NETWORK.LOCALNET"
...
step_setup_environment() {
  ...
  if command -v docker >/dev/null 2>&1; then
    # Check if Docker daemon is actually responding (socket may exist even if daemon is down)
    local _docker_live
    _docker_live="$(python3 -c "
import socket
for path in ['/var/run/docker.sock', __import__('os').path.expanduser('~/.docker/run/docker.sock')]:
  try:
...
" 2>/dev/null || true)"
  ...
  local DEF_ADMIN_ROLE="0x97667070c54ef182b0f5858b034beac1b6f3089aa2d3188bb1e8929f4fa9b929" # TSS_MANAGER_ROLE
...
async function main() {
  ...
  // Derive both PDA addresses: tsspda_v2 (legacy program seed) and final_tss_pda (new seed
  // read by the Go universal validator binary).
  const [legacyPda] = PublicKey.findProgramAddressSync([Buffer.from('tsspda_v2')], programId);
  const [finalPda] = PublicKey.findProgramAddressSync([Buffer.from('final_tss_pda')], programId);

  const account = await connection.getAccountInfo(legacyPda);
  if (!account) {
    throw new Error(`Solana gateway TSS PDA ${legacyPda.toBase58()} is not initialized`);
  }
  ...
  async function injectAt(address) {
    const resp = await fetch(rpc, {
      ...
      body: JSON.stringify({
        ...
        params: [address, snapshot],
      }),
    });
    const payload = await resp.json();
    if (payload.error) {
      throw new Error(`surfnet_setAccount(${address}) failed: ${payload.error.message || JSON.stringify(payload.error)}`);
    }
  }

  if (current.toLowerCase() === desired.toLowerCase()) {
    console.log(`Solana gateway TSS already matches ${desired} (chain_id=${chainId}) on legacy PDA`);
  } else {
    await injectAt(legacyPda.toBase58());
    const updated = await connection.getAccountInfo(legacyPda);
  ...
  // Also inject at the final_tss_pda address so the Go universal validator binary
  // (which derives TSS PDA with seed "final_tss_pda") can read it.
  await injectAt(finalPda.toBase58());
  console.log(`Solana gateway TSS also injected at final_tss_pda ${finalPda.toBase58()}`);
}
...
step_deploy_local_sol_usdt_prc20() {
  ...
  for _attempt in 1 2 3; do
    ...
    [[ $_attempt -lt 3 ]] && { log_info "PRC20 impl deploy attempt $_attempt failed, retrying in 3s..."; sleep 3; }
  done
  if ! echo "$impl_out" | grep -q "Deployed to:"; then
  ...
  init_data="$(cast calldata 'initialize(string,string,uint8,string,uint8,uint256,address,string)' \
    'USDT.sol' \
    'USDT.sol' \
    6 \
  ...
  # Brief pause to let the impl deployment settle before deploying the proxy
  sleep 2
  proxy_out=""
  for _attempt in 1 2 3; do
    proxy_out="$(
    ...
    [[ $_attempt -lt 3 ]] && { log_info "PRC20 proxy deploy attempt $_attempt failed, retrying in 3s..."; sleep 3; }
  done
  if ! echo "$proxy_out" | grep -q "Deployed to:"; then
    log_err "Failed to deploy USDT.sol PRC20 proxy after 3 attempts"
    echo "$proxy_out"
    exit 1
  fi
...
step_setup_gateway() {
  ...
  if ! cast call "$C1" 'universalCore()(address)' --rpc-url "$PUSH_RPC_URL" >/dev/null 2>&1; then
    log_err "C1.universalCore() reverted. Gateway localSetup script did not initialize C1 properly."
  ...
  c1_uc="$(cast call "$C1" 'universalCore()(address)' --rpc-url "$PUSH_RPC_URL" 2>/dev/null || true)"
  ...
  if [[ -n "$c1_uc" && -n "$c0_ug" ]]; then
    ...
    if [[ "$c1_uc_tmp" == "$c0_lc_tmp" && "$c0_ug_tmp" != "$c1_lc_tmp" ]]; then
      ...
      cast send "$C0" 'updateUniversalGatewayPC(address)' "$C1" \
        --rpc-url "$PUSH_RPC_URL" \
  ...
  if [[ "$c1_uc_lc" != "$c0_lc" || "$c0_ug_lc" != "$c1_lc" ]]; then
    log_err "Gateway wiring invalid after setup: C1.universalCore=$c1_uc, C0.universalGatewayPC=$c0_ug"
    exit 1
  fi
  ...
  if [[ -s "$DEPLOY_ADDRESSES_FILE" ]]; then
    while IFS=$'\t' read -r symbol token_addr; do
      ...
      cast send "$C0" 'updateGasTokenPRC20(string,address)' "$chain_ns" "$token_addr" \
        --rpc-url "$PUSH_RPC_URL" \
        --private-key "$PRIVATE_KEY" >/dev/null || true
  ...
  if [[ -z "$base_gas" || "$base_gas" == "0" ]]; then
    ...
    for ns in "eip155:11155111" "eip155:421614" "eip155:84532" "eip155:97" "solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1"; do
      cast send "$C0" 'updateBaseGasLimitByChain(string,uint256)' "$ns" "$LOCAL_OUTBOUND_BASE_GAS_LIMIT" \
        --rpc-url "$PUSH_RPC_URL" \
        --private-key "$PRIVATE_KEY" >/dev/null || true
    done
  fi
...
step_add_uregistry_configs() {
  ...
  while IFS= read -r token_json; do
    ...
    if is_local_testing_env &&
      ...
      # Use the full 20-byte zero address (matches SDK NATIVE constant and the
      # universalClient native-SOL check). The security-audit-fixes node canonicalizes
      # solana token addresses and rejects the odd-length "0x0" shorthand.
      native_alias_payload="$(echo "$token_payload" | jq -c '.address = "0x0000000000000000000000000000000000000000"')"
      log_info "Adding local Solana native SOL alias to uregistry: 0x0000000000000000000000000000000000000000"
...
main() {
  ...
  case "$cmd" in
    ...
    quick-testing-outbound-evm) step_run_sdk_quick_testing_outbound_evm ;;
    quick-testing-outbound-svm) step_run_sdk_quick_testing_outbound_svm ;;
