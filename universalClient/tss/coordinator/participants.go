...
type StaticParticipantProvider struct {
	participants map[string][]tss.Participant // protocolType -> participants
...
func NewStaticParticipantProvider(logger zerolog.Logger) *StaticParticipantProvider {
	return &StaticParticipantProvider{
		participants: make(map[string][]tss.Participant),
		logger:       logger,
	}
}

// SetParticipants sets the participants for a protocol type.
func (p *StaticParticipantProvider) SetParticipants(protocolType string, participants []tss.Participant) {
	// Sort by PartyID for consistency
	sorted := make([]tss.Participant, len(participants))
...
func (p *StaticParticipantProvider) GetParticipants(ctx context.Context, protocolType string, blockNumber uint64) ([]tss.Participant, error) {
	...
	result := make([]tss.Participant, len(participants))
