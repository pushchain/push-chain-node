...
func (c *Chains) addChain(ctx context.Context, cfg *uregistrytypes.ChainConfig) error {
	...
	case uregistrytypes.VmType_EVM:
		client, err = evm.NewClient(cfg, chainDB, chainConfig, c.pushSigner, c.config.AllowsZeroConfirmationsForChain(cfg.Chain), c.logger)
	case uregistrytypes.VmType_SVM:
		client, err = svm.NewClient(cfg, chainDB, chainConfig, c.pushSigner, c.config.NodeHome, c.config.AllowsZeroConfirmationsForChain(cfg.Chain), c.logger)
	default:
